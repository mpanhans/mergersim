...
<!--
To do: move comparisons to antitrust package to a separate testing file.
-->

# Introduction

Simple models of competition that can be calibrated with only a few inputs can be useful to readily analyze certain mergers. For example, such an exercise can be useful when screening for possible concern in a transaction. Even with simple models, however, there are several nuances to consider. This document covers a Bertrand model and a second score auction model, illustrates how to do a merger simulation, and includes all the R code necessary to reproduce the analysis.
