...
A straightforward model of a vertical supply chain like in Sheu and Taragin (2021) ``Simulating mergers in a vertical supply chain with bargaining'' can be used to evaluate effects of vertical mergers. That model uses a simultaneous timing assumption for equilibrium. This document covers the Sheu-Taragin model, illustrates how to do a simple merger simulation, and includes all the R code necessary to reproduce the analysis.
...
out1 <- multiroot(f = bertrand_foc_novert ,start = p_r_start, 
...
out1b <- BBoptim(f = bertrand_foc_novert, par = p_r_start, 
                  own_down = own_down_pre, alpha= alpha, 
                  delta = delta, cost = c_R_vec,
...
bargain_foc_novert_sim(price_w = x0, own_down = own_down_pre, own_up = own_up_pre, 
         alpha= alpha, delta = delta, 
         cost_w = c_W_vec, cost_r = c_R_vec, 
         lambda = lambda, price_r = (p1*2))

out2 <- BBoptim(par = x0, fn = bargain_foc_novert_sim, 
...
In equilibrium, retail prices and wholesale prices need to be jointly determined. The functions defined above can be simultaneously solved to find equilibrium retail and wholesale prices of the model.
...
while (error > tol) {
  ...
  # p_R1 <- out1$root
  # BBoptim:
  out1 <- BBoptim(f = bertrand_foc_novert, par = p_R0, 
                  own_down = own_down_pre, alpha= alpha, 
                  delta = delta, cost = c_R_vec,
  ...
  out2 <- BBoptim(par = as.numeric(p_W0), fn = bargain_foc_novert_sim, 
                  own_down = own_down_pre, own_up = own_up_pre, 
                  alpha= alpha, delta = delta, 
...
# GFT
bargain_foc_novert_sim(price_w = p_W1, own_down = own_down_pre, own_up = own_up_pre, 
         alpha= alpha, 
...
bertrand_foc_novert(price_r = p_R1, own_down = own_down_pre, alpha = alpha, 
...
out3 <- BBoptim(f = bertrand_vert_calibrate_v2, par = x00, 
                   own_down = own_down_pre, price = p_R1, 
                   shares = shares1, cost  = c_R_vec, p_W = p_W1,
                   weight = wt_matrix)
```
...
## Alternative calibration method of downstream model

In the above calibration, the optimization searches across all demand parameters simultaneously, finding those that best match the FOCs and share equations. A weighting matrix needs to be specified to determine how much relative importance to place on matching the share equations relative to the pricing equations. When the data does not perfectly fit the model, the relative weights matter a great deal for the results.

An alternative approach can find mean value parameters that match the observed shares exactly, and then only optimize over the price coefficient to find the parameter that best fits the first order conditions. This implicitly takes a stand that the share equations have absolute priority over the FOCs. This can be useful in some cases, such as when there are many products, and search over the mean values is slow. Uses match_share_logit and bertrand_vert_calibrate
...
out3b <- BBoptim(f = bertrand_vert_calibrate, par = x00b, 
                   own_down = own_down_pre, price = p_R1, 
                   shares = shares1, cost  = c_j, price_w = p_W1)

alpha3b <- out3b$par
deltaout <- multiroot(f = match_share_logit, start = c(.5,.5,.5,.5,.5,.5),
                      shares = shares1, alpha = alpha3b, price = p_R1)
delta3b <- deltaout$root
...
And finally consider yet another function that uses the Berry (1994) inversion to match shares exactly, rather than numerically matching them using multiroot. (The code block below is now mostly redundant to the above. Could just show the berry inversion gives same result as numerical search.)
...
out3c <- BBoptim(f = bertrand_vert_calibrate, par = x00b, 
                   own_down = own_down_pre, price = p_R1, 
                   shares = shares1, cost  = c_j, price_w = p_W1)

alpha3c <- out3c$par
delta3c <- log(shares1) - log(1-sum(shares1)) - alpha3c*p_R1
...
x00b <- rep(1,J)
bertrand_vert_calibrate_costs(param = x00b, 
                   own_down = own_down_pre, price = p_R1, 
                   shares = shares1, alpha = alpha, delta = delta,
                   price_w = p_W1 )


out3e <- BBoptim(f = bertrand_vert_calibrate_costs, par = x00b, 
                   own_down = own_down_pre, price = p_R1, 
                   shares = shares1, alpha = alpha, delta = delta,
                   price_w = p_W1)
```
